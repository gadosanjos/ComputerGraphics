...
/* 
    ...
    On the first call, resize() will be called with width = 600 and height = 800 
...
*/
    ...
    glOrtho(-2.5,2.75,0,10.5, -1.0, 1.0); // adjusted for suitable viewport/Define orthographic projection
    //glOrtho(-2.5,2.75,0,10.5, -1.0, 1.0); //whole view
    //glOrtho(0,2.75,0,10.5, -1.0, 1.0);    //middle of the stem
    //glOrtho(2.75,5.25,0,10.5, -1.0, 1.0); // pass the fern x points
    //glOrtho(-1, 1, -1, 1, -1, 1);      // You would now be focusing on a smaller area, essentially zooming in, because the x- and y-ranges are smaller.
    //glOrtho(-10, 10, -10, 10, -1, 1);  // This would zoom out, as you are displaying a wider area on the screen.
}
